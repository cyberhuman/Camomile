...
let look s i =
  let n' =
    ...
    if n <= 0xdf then
      ...
      let n' = n' lsl 6 lor (0x7f land m) in
      n' lsl 6 lor (0x7f land m0)     
    else invalid_arg "UTF8Strict.look"
  in
  UChar.of_int n'
...
let next s i = 
  ...
  else invalid_arg "UTF8Strict.next"
...
let add_uchar buf u =
  ...
  if k < 0 || k > 0x10ffff then invalid_arg "UTF8Strict.add_uchar"
  else if k <= 0x7f then
    ...
    if k >= 0xd800 && k <= 0xdfff then invalid_arg "UTF8Strict.add_uchar";
...
let rec length_aux s c i =
  ...
  let k =
    ...
    if n < 0xc0 then invalid_arg "UTF8Strict.length" else
    if n < 0xe0 then 2 else
    if n < 0xf0 then 3 else
    if n < 0xf8 then 4 else
    invalid_arg "UTF8Strict.length" in
...
let validate s =
  ...
  let rec main i =
    ...
    if n <= 0xdf then 
      ...
      let trail = trail 2 (i + 1) (n - 0xe0) in
      if trail < 0x800 || (trail >= 0xd800 && trail <= 0xdfff) then raise Malformed_code else 
      main (i + 3)
    else if n <= 0xf7 then 
      let trail = trail 3 (i + 1) (n - 0xf0) in
      if trail < 0x10000 || trail > 0x10ffff then raise Malformed_code else
      main (i + 4)
